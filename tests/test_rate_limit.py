...
class TestParseRetryAfter:
    ...
    def test_valid_integer(self):
        ...
        assert SteamAPIClient._parse_retry_after(response) == 120.0

    def test_valid_float(self):
        response = MagicMock()
        response.headers = {"Retry-After": "30.5"}
        assert SteamAPIClient._parse_retry_after(response) == 30.5

    def test_missing_header(self):
        response = MagicMock()
        response.headers = {}
        assert SteamAPIClient._parse_retry_after(response) == 60.0
    ...
    def test_zero_value_clamps_to_one(self):
        response = MagicMock()
        response.headers = {"Retry-After": "0"}
        assert SteamAPIClient._parse_retry_after(response) == 1.0

    def test_negative_value_clamps_to_one(self):
        response = MagicMock()
...
class TestGetSteamId:
    ...
    def test_valid_steam64_id(self):
        steam_id = "76561198000000001"
        assert SteamAPIClient.get_steam_id(steam_id) == steam_id

    def test_friend_code_to_steam64(self):
        # 76561197960265729 - 76561197960265728 = 1
        result = SteamAPIClient.get_steam_id("1")
        assert result == "76561197960265729"

    def test_non_digit_returns_none(self):
        assert SteamAPIClient.get_steam_id("STEAM_0:1:xxxx") is None

    def test_empty_string_returns_none(self):
        assert SteamAPIClient.get_steam_id("abc") is None
...
class TestRequestWithRetry:
    ...
    @pytest.mark.asyncio
    async def test_success_on_first_try(self, app: App):
        """200 成功响应应直接返回。"""
        client = _make_client()
    ...
    @pytest.mark.asyncio
    async def test_429_retry_then_success(self, app: App):
        """429 限流后应等待并重试，最终成功。"""
        client = _make_client()
    ...
    @pytest.mark.asyncio
    async def test_429_exhausts_retries(self, app: App):
    ...
    @pytest.mark.asyncio
    async def test_403_switches_key(self, app: App):
        """403 认证失败应切换到下一个 key。"""
        client = _make_client(api_keys=["bad_key", "good_key"])
    ...
    @pytest.mark.asyncio
    async def test_5xx_retries_with_backoff(self, app: App):
        """5xx 服务端错误应指数退避后重试。"""
        client = _make_client()
    ...
    @pytest.mark.asyncio
    async def test_network_error_retries(self, app: App):
        """网络错误应指数退避后重试。"""
        client = _make_client()
        success = _make_response(200, GOOD_RESPONSE)
    ...
    @pytest.mark.asyncio
    async def test_all_keys_fail_returns_none(self, app: App):
        """所有 key 都失败应返回 None。"""
...
class TestGetUsersInfo:
    ...
    @pytest.mark.asyncio
    async def test_empty_steam_ids(self, app: App):
        """空 Steam ID 列表应直接返回空。"""
        client = _make_client()
    ...
    @pytest.mark.asyncio
    async def test_all_keys_fail_returns_empty(self, app: App):
    ...
    @pytest.mark.asyncio
    async def test_batch_splitting(self, app: App):
        """超过 100 个 Steam ID 应分批请求。"""
        client = _make_client(batch_delay=0.001)
